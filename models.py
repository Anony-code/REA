...
class SimpleNN_gan(object):
    ...
    def build(self):
        ...
        # self.sample = tf.expand_dims(tf.argmax(probs, axis=1), axis=1)
        # _, self.sample = tf.nn.top_k(probs, self.n_sample)
        ...
        # self.opt = tf.train.GradientDescentOptimizer(learning_rate=0.02).minimize(self.cost)
    ...
    def generate(self, neg_head, neg_tail, sess, row_idx, new_neg, n_sample=1, nn=1):
        ...
        for i in range(sample.shape[1]):
            # idx = np.concatenate((row_idx, np.expand_dims(sample[:, i], axis=1).astype(int)), axis=1)
            # neg = np.expand_dims(tf.gather_nd(new_neg, idx), axis=1)
        ...
        # neg_out = np.transpose(np.concatenate(np.expand_dims(neg_list, axis=1), axis=1))
...
class SimpleNN_clf(object):
    ...
    def build(self):
        ...
        self.cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.sample,
                                   ...
                                   # 0.001*(tf.nn.l2_loss(self.weights['h1']) + tf.nn.l2_loss(self.weights['out']) +
                                   # tf.nn.l2_loss(self.biases['b1']) + tf.nn.l2_loss(self.biases['out'])))
        if self.mode == "clf":
            self.opt = tf.train.AdamOptimizer(learning_rate=0.01).minimize(self.cost, var_list=[self.weights, self.biases])
        else:
            self.opt = tf.train.GradientDescentOptimizer(learning_rate=FLAGS.learning_rate).minimize(self.cost)
    ...
    def train_classifier_step(self, pos_score, neg_score, target, classifier, sess):
        ...
        # new_left = np.concatenate((neg_left, neg_left))
        # new_right = np.concatenate((neg2_right, neg2_right))
        # pos = np.concatenate((new_left.reshape((len(new_left), 1)), new_right.reshape((len(new_right), 1))), axis=1)
        # pos = np.reshape(pos, (new_neg.shape[0], -1))[:, :2]
        ####
        # vec_se = sess.run(model.outputs, feed_dict=feed_dict_se)
        # pos_head, pos_tail = self.get_embedding(vec_se, tf.cast(pos, tf.int32))
        # pos_score = self.forward(tf.abs(pos_head - pos_tail))
        # pos_score = pos_head - pos_tail
        #         # new_pos_for_neg = np.expand_dims(np.reshape(np.concatenate((neg_left, neg2_right)), (new_neg.shape[0], -1))[:, 0],
        #         #                                  axis=1).astype(int)
        # neg = tf.concat((new_pos_for_neg, new_neg), axis=1)

        ####
        # neg_head, neg_tail = self.get_embedding(vec_se, tf.cast(neg, tf.int32))
        # neg_score = self.forward(tf.abs(neg_head - neg_tail))
        # neg_score = neg_head - neg_tail
        # target = tf.concat([tf.ones(pos_score.shape[0]), tf.zeros(neg_score.shape[0])], axis=0)
    ...
    def discriminator_step(self, pos, neg, discriminator, vec_se, sess):
        ...
        # _, loss, pos_, neg_prob, neg_pred = sess.run([discriminator.opt, discriminator.cost, discriminator.pos_score,
        loss, pos_, neg_prob, neg_pred = sess.run([discriminator.cost, discriminator.pos_score,
                                        discriminator.neg_score_probs, discriminator.neg_score],
    ...
    def train_gan_step(self, neg_head, neg_tail, row_idx, new_neg, new_pos_for_neg, pos,
        ...
        # new_neg, row_sample_idx, score = generator.generate(model, feed_dict_se, sess, neg_left, neg_right, neg2_left, neg2_right)
        # new_left = np.concatenate((neg_left, neg_left))
        # new_right = np.concatenate((neg2_right, neg2_right))
        # pos = np.concatenate((new_left.reshape((len(new_left), 1)), new_right.reshape((len(new_right), 1))), axis=1)
        # pos = np.reshape(pos, (new_neg.shape[0], -1))[:, :2]
        ...
        # feed_left = np.concatenate((neg_left, neg2_left))
        # feed_right = np.concatenate((neg_right, neg2_right))
        # feed = np.concatenate((feed_left.reshape((len(feed_left), 1)), feed_right.reshape((len(feed_right), 1))), axis=1)
        # vec_se = sess.run(model.outputs, feed_dict=feed_dict_se)
        # neg_head, neg_tail = self.get_embedding(vec_se, feed.astype(int))
    ...
    def find_topK_pair(self, model, feed_dict_se, classifier, train_dataset, clf_dataset, gan_dataset, sess, k = None, k1 = None):
        ...
        while i < len(train_dataset.train):
            ...
            weight = (np.asarray(orig_weight) >= 0.1).astype(float)    # changed from 0.5 to 0.1
            ...
            # score = - np.reshape(np.asarray(orig_weight), (1, -1))[0]
            # score = score.eval(session=sess)

            for x, pair in enumerate(train_dataset.train[i:j]):
                if pair not in train_dataset.pretrain:
                    train_dataset.subsampling_weight[pair] = weight[0][x][0]

                    if weight[0][x][0] > 0.5:
                        new_set_discrim.append(pair)

                topk_heap.push((score[x], pair))
        ...
        print('\n' + 'total entity pairs for disc: ' + str(len(clf_dataset.train)))
        get_clf_gan_label(train_dataset, clf_dataset, 'clf')
        clf_dataset.orgin_train = np.asarray([[item[0], item[1]] for item in clf_dataset.train])
        _, gan_dataset.train = list(zip(*bottomK_list))
        get_clf_gan_label(train_dataset, gan_dataset, 'gan')
